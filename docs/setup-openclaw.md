# Setting Up AIperture with OpenClaw

This guide walks you through wiring AIperture as the permission layer for [OpenClaw](https://github.com/ClawDBot/openclaw), an open-source AI agent runtime. After setup, OpenClaw will check every tool call through AIperture, and AIperture will learn your approval patterns over time.
...
```bash
pip install aiperture             # AIperture
```
...
> **Why the low thresholds?** `3` decisions and `0.80` approval rate let you see the learning loop in action quickly. For real use, bump these to the defaults (`10` decisions, `0.95` threshold) or run `aiperture configure` to tune them.

## Step 4: Add the system prompt

Create `system_prompt.md` in the same directory. This tells OpenClaw how to use AIperture's tools.

The key distinction: AIperture is **only for tool use permissions** (reading files, running commands, API calls). It should NOT be involved when the agent is asking you a clarifying question, presenting options, or having a normal conversation.
...
When you start your first OpenClaw session with AIperture:

1. **Every action starts as denied** — AIperture has no history, so the first time the agent tries to read a file, run a command, or call an API, AIperture denies it. The agent will ask for your approval.
...
3. **AIperture counts your decisions** — With the demo thresholds above (`3` decisions, `0.80` rate), it only takes 3 approvals before AIperture starts auto-approving that action type. With production defaults, it takes 10.

4. **Auto-approval kicks in** — Once AIperture has seen enough consistent approvals, it returns `ALLOW` with `decided_by: auto_learned`. The agent proceeds without asking you. You'll notice the prompts getting less frequent as the session goes on.
...
### First time — AIperture denies (no history)
...
Agent:  "AIperture denied this action (no prior history). It's low risk —
...
Agent:  "AIperture auto-approved this based on your previous decisions.
...
Agent:  "AIperture flagged this as HIGH risk. Do you want to approve it?"
```

## Checking what AIperture learned

Ask the agent:

```
You:    "Show me what AIperture has learned"

Agent:  → get_permission_patterns(min_decisions=3, organization_id="default")

Agent:  "AIperture has learned 2 patterns:
...
Or query the REST API directly (if `aiperture serve` is running):
...
This runs the full deny → approve → auto-approve cycle in-process using AIperture's API.
...
If you also run `aiperture serve` for querying alongside the MCP server, you can secure it with a bearer token:
...
All HTTP API requests must then include `Authorization: Bearer your-secret-key`. The MCP server (used by OpenClaw) is unaffected — it uses stdio transport.

You can check database health at any time:
...
For real use, update your `openclaw.json` env or `.aiperture.env`:
...
The system prompt is what tells OpenClaw to use AIperture's tools. Make sure `system_prompt.md` is in the same directory and your OpenClaw config references it.
